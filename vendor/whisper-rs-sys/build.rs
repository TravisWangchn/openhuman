...
fn main() {
    ...
    // On Windows GNU targets, force cmake to use MinGW Makefiles generator
    // so it picks up gcc/g++ from WinLibs instead of MSVC/clang.
    // Also set the static library prefix to "lib" so GNU ld finds the .a files.
    #[cfg(all(target_os = "windows", target_env = "gnu"))]
    {
        config.generator("MinGW Makefiles");
        // Explicitly set compilers to avoid cmake picking up LLVM clang
        // which targets MSVC ABI and rejects -fPIC.
        config.define("CMAKE_C_COMPILER", "gcc");
        config.define("CMAKE_CXX_COMPILER", "g++");
        config.define("CMAKE_STATIC_LIBRARY_PREFIX", "lib");
        config.define("CMAKE_STATIC_LIBRARY_SUFFIX", ".a");
    }
    ...
    // On Windows GNU, cmake generates .a files without the "lib" prefix
    // that GNU ld expects (e.g. ggml.a instead of libggml.a).
    // Copy them so the linker can find them.
    #[cfg(all(target_os = "windows", target_env = "gnu"))]
    {
        use std::fs;
        fn copy_lib_prefixed(dir: &std::path::Path) -> std::io::Result<()> {
            if dir.is_dir() {
                for entry in fs::read_dir(dir)? {
                    let entry = entry?;
                    let path = entry.path();
                    if path.is_dir() {
                        copy_lib_prefixed(&path)?;
                    } else if path.extension().map_or(false, |e| e == "a") {
                        let fname = path.file_name().unwrap().to_str().unwrap();
                        if !fname.starts_with("lib") {
                            let parent = path.parent().unwrap();
                            let new_name = format!("lib{}", fname);
                            let new_path = parent.join(&new_name);
                            if !new_path.exists() {
                                fs::copy(&path, &new_path)?;
                                println!("cargo:warning=Copied {} -> {}", path.display(), new_path.display());
                            }
                        }
                    }
                }
        ...
        if let Err(e) = copy_lib_prefixed(&build_dir) {
            println!("cargo:warning=Failed to copy lib-prefixed .a files: {}", e);
        }
    }
